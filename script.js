                        ...
                        <p>
                            ${
                                job.description.startsWith("http")
                                    ? `<a href="${job.description}" target="_blank" onclick="showReminder()">Job Description</a>`
                                    : job.description
                            }
                        </p>
...
function applyJob(jobId, button) {
    ...
    .then(({ status, body }) => {
        if (status === 200) {
            showSuccess(); // ✅ Show success popup instead of alert
